...
class _EditTransactionScreenState extends State<EditTransactionScreen> {
  ...
  void initState() {
    ...
    // Cargamos las cuentas del usuario
    _accountsFuture = supabase.from('accounts').select('id, account_name').eq('user_id', supabase.auth.currentUser!.id);
  }

  Future<void> _updateTransaction() async {
    if (!_formKey.currentState!.validate()) return;
    if (_selectedCategory == null || _selectedAccountId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Por favor, selecciona una categoría y una cuenta.'), backgroundColor: Colors.orange));
      return;
    }
    ...
    try {
      await supabase.from('transactions').update({
        ...
        'transaction_date': widget.transaction['transaction_date'] // Mantenemos la fecha original
      ...
      if (mounted) {
        ...
        Navigator.of(context).pop(true); // Devolvemos true para indicar que hubo cambios
      }
    } catch (e) {
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: (value) => (value == null || value.isEmpty || double.tryParse(value) == null) ? 'Ingresa un monto válido' : null,
              ),
              const SizedBox(height: 24),
              // --- SELECTOR DE TIPO (GASTO/INGRESO) ---
              SegmentedButton<String>(
                segments: const [
                  ButtonSegment<String>(value: 'Gasto', label: Text('Gasto'), icon: Icon(Iconsax.arrow_down_2)),
                  ButtonSegment<String>(value: 'Ingreso', label: Text('Ingreso'), icon: Icon(Iconsax.arrow_up_1)),
                ],
                selected: {_transactionType},
                onSelectionChanged: (newSelection) {
                  setState(() {
                    _transactionType = newSelection.first;
                    // Si la categoría seleccionada no existe en la nueva lista, la reseteamos.
                    if (!_currentCategories.keys.contains(_selectedCategory)) {
              ...
              FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError || !snapshot.hasData) return const Text('No se pudieron cargar las cuentas.');
                  
                  final accounts = snapshot.data!;
                  return DropdownButtonFormField<int>(
                    value: _selectedAccountId,
                    items: accounts.map((account) => DropdownMenuItem<int>(
                      value: account['id'],
              ...
              Wrap(
                ...
                children: _currentCategories.entries.map((entry) {
                  return ChoiceChip(
                    ...
                    onSelected: (selected) => setState(() => _selectedCategory = entry.key),
                  );
                }).toList(),
              ),
              const SizedBox(height: 24),
