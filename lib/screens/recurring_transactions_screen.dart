  ...
  Future<void> _handleDelete(RecurringTransaction item) async {
    ...
    try {
      ...
      NotificationHelper.show(
          message: 'Eliminado', type: NotificationType.success);
    } catch (_) {
      HapticFeedback.mediumImpact();
      NotificationHelper.show(
      ...
      child: Scaffold(
        ...
        body: NestedScrollView(
          ...
          body: FutureBuilder<List<Category>>(
              // 👈 NUEVO: Envolvemos en FutureBuilder de Categorías
                    ...
                    return TabBarView(
                      ...
                      children: [
                        ...
                        _RecurringList(
                          ...
                          items:
                              items.where((i) => i.type == 'Ingreso').toList(),
                          categories: categories, // 👈 Pasamos las categorías
                          isExpense: false, c: c,
                          repository: _repository,
                          onEdit: _navigateToEdit,
...
class _RecurringList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingCount = items.where((i) {
      final itemDate =
          DateTime(i.nextDueDate.year, i.nextDueDate.month, i.nextDueDate.day);
        ...
        slivers: [
          SliverToBoxAdapter(
            child: _FadeSlide(
              delay: 0,
              child:
                  _MonthlySummaryCard(items: items, isExpense: isExpense, c: c),
            ),
          ),
            ...
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (_, i) => _FadeSlide(
                  delay: 60 + i * 45,
                  child: _PaymentCard(
                    item: overdue[i], categories: categories, c: c,
                    repository: repository, // 👈 Pasamos categorías
            ...
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (_, i) => _FadeSlide(
                  delay: 70 + i * 45,
                  child: _PaymentCard(
                    item: dueToday[i], categories: categories, c: c,
                    repository: repository, // 👈 Pasamos categorías
              ...
              sliver: SliverList(
                delegate: SliverChildBuilderDelegate(
                  (_, i) => _FadeSlide(
                    delay: 80 + i * 45,
                    child: _PaymentCard(
                      item: upcoming[i], categories: categories, c: c,
                      repository: repository, // 👈 Pasamos categorías
...
class _MonthlySummaryCard extends StatelessWidget {
  ...
  double get _monthly => items.fold<double>(0, (sum, item) {
        ...
        final double factorMensual = switch (item.frequency.toLowerCase()) {
          ...
          'mensual' => 1.0,
          'bimestral' =>
            0.5, // Bimestral: se paga cada 2 meses (0.5 veces al mes)
        ...
        children: [
          ...
          Column(
            ...
            children: [
              _MiniStat(
                  label: 'Diario',
                  value: fmt.format(total / 30),
                  color: color,
                  c: c),
              const SizedBox(height: _C.sm),
              _MiniStat(
                  label: 'Anual',
                  value: compactFmt.format(total * 12),
                  color: color,
                  c: c),
            ],
          ),
...
class _PaymentCardState extends State<_PaymentCard>
  ...
  Category? _findCategory() {
    try {
      ...
      return widget.categories.firstWhere(
          (cat) => cat.name == widget.item.category && cat.type == typeEnum);
    } catch (_) {
      return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final catColor =
        cat != null ? cat.colorAsObject : (_isExpense ? _C.expense : _C.income);
    final catIcon =
        cat?.icon ?? (_isExpense ? Iconsax.receipt_minus : Iconsax.receipt_add);
          ...
          child: Container(
            ...
            child: Column(
              children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(children: [
                              Text(_freqLabel,
                                  style:
                                      TextStyle(fontSize: 13, color: c.label3)),
                              const SizedBox(width: 6),
                              Container(
                                  width: 3,
                                  height: 3,
                                  decoration: BoxDecoration(
                                      shape: BoxShape.circle, color: c.label4)),
                              const SizedBox(width: 6),
                              Container(
                ...
                Padding(
                  padding:
                      const EdgeInsets.fromLTRB(_C.md, _C.sm, _C.md, _C.sm),
                  child: Row(
                    children: [
                      _QuickAction(
                        label: 'Omitir',
                      ...
                      Expanded(
                        child: _PrimaryActionBtn(
                          label:
                              _isExpense ? 'Pagar ahora' : 'Registrar ingreso',
                          color: catColor,
                          onTap: _pay,
                        ),
                      ),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: _pressing ? widget.color.withOpacity(0.8) : widget.color,
          borderRadius: BorderRadius.circular(_C.rMD),
        ),
        alignment: Alignment.center,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                // 👈 NUEVO: Mostrar Beneficiario y Cuenta en los detalles
                if ((item.payeeName != null && item.payeeName!.isNotEmpty) ||
                    (item.payeeAccount != null &&
                        item.payeeAccount!.isNotEmpty)) ...[
                  const Spacer(), // Empujamos el botón de cerrar a la derecha
                ],
                if ((item.payeeName == null || item.payeeName!.isEmpty) &&
                    (item.payeeAccount == null ||
                        item.payeeAccount!.isEmpty)) ...[
                  const Spacer(),
                ],
              ],
            ),
          ...
          // 👈 BLOQUE NUEVO VISUAL PARA LOS DATOS BANCARIOS / CONTACTO
          if ((item.payeeName != null && item.payeeName!.isNotEmpty) ||
              (item.payeeAccount != null && item.payeeAccount!.isNotEmpty))
            Container(
              margin: const EdgeInsets.fromLTRB(_C.lg, 0, _C.lg, _C.md),
              ...
              child: Row(
                children: [
                  Icon(Iconsax.bank,
                      size: 18, color: isExpense ? _C.expense : _C.income),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        if (item.payeeName != null &&
                          ...
                          Text(item.payeeName!,
                              style: TextStyle(
                                  ...
                                  fontWeight: FontWeight.w600,
                                  color: c.label)),
                        if (item.payeeAccount != null &&
                            item.payeeAccount!.isNotEmpty)
                          Text(item.payeeAccount!,
          ...
          _SheetOption(
            ...
            color: isExpense ? _C.expense : _C.income,
            c: c,
            onTap: () {
          ...
          _SheetOption(
            icon: Iconsax.next,
            label: 'Omitir período',
            subtitle: 'Saltar al siguiente ciclo',
            color: _C.warning,
            c: c,
            onTap: () {
              Navigator.pop(context);
          ...
          Container(
            margin:
                const EdgeInsets.symmetric(horizontal: _C.lg, vertical: _C.sm),
            height: 0.5,
            color: c.separator,
          ),
          _SheetOption(
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 40,
              decoration: BoxDecoration(
                color: widget.color.withOpacity(widget.c.isDark ? 0.18 : 0.1),
        ...
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: Container(
                padding:
                    const EdgeInsets.symmetric(horizontal: _C.lg, vertical: 13),
                decoration: BoxDecoration(
                    color: _C.accent,
                    borderRadius: BorderRadius.circular(_C.rMD)),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Algo salió mal',
                style: TextStyle(
                    fontSize: 20, fontWeight: FontWeight.w700, color: c.label)),
            const SizedBox(height: _C.sm),
            Text(error,
                textAlign: TextAlign.center,
      ...
      builder: (_, __) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) => Container(
            ...
            height: 120,
            decoration: BoxDecoration(
                color: shimmer, borderRadius: BorderRadius.circular(_C.rXL)),
          ),
        );
      },
    );
...
class _PendingAlertBanner extends StatelessWidget {
  ...
  const _PendingAlertBanner(
      {required this.count, required this.c, required this.onTap});
