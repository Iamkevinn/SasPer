...
class EditGoalScreen extends StatefulWidget {
  ...
  @override State<EditGoalScreen> createState() => _EditGoalScreenState();
}
...
class _EditGoalScreenState extends State<EditGoalScreen>
  ...
  void initState() {
    ...
    _targetCtrl = TextEditingController(text: g.targetAmount.toStringAsFixed(0));
    ...
    _notificationTime = TimeOfDay(
        hour:   g.notificationHour,
        minute: g.notificationMinute);
  ...
  Future<void> _loadFinancialData() async {
    try {
      ...
      if (summaries.isNotEmpty) {
        _realMonthlyIncome = summaries.fold<double>(
                0.0, (s, e) => s + e.totalIncome) /
            summaries.length;
      }
    } catch (e) {
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _categories = cats.where((c) => c.type == CategoryType.expense).toList();
        });
      }
    } catch (e) {
  ...
  void _openCategorySheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => DraggableScrollableSheet(
        initialChildSize: 0.55, maxChildSize: 0.85, expand: false,
  ...
  void _applySuggestion(RecommendationType type) {
    ...
    switch (type) {
      ...
      case RecommendationType.reduceTarget:
        _targetCtrl.text =
            (widget.goal.targetAmount * 0.8).toStringAsFixed(0);
        break;
      case RecommendationType.increaseContribution:
        break;
    }
    _recalculate();
    NotificationHelper.show(message: 'Sugerencia aplicada', type: NotificationType.info);
  }
...
Future<void> _save() async {
    ...
    try {
      ...
      final currentMonthly = remaining / math.max(1, (safeDays / 30.437).ceil());
  ...
  Future<bool?> _showConfirmSheet({
    required String newName, required double newAmount,
    required bool nameChanged, required bool amountChanged,
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Container(
                  ...
                  child: const Center(child: Icon(
                      Iconsax.info_circle, size: 22, color: _kBlue)),
                ),
                ...
                Text('No modificaste ningún campo.',
                    ...
                    style: _T.label(14, c: onSurf.withOpacity(0.50),
                        w: FontWeight.w400)),
                const SizedBox(height: 24),
                _SheetBtn(label: 'Entendido', color: _kBlue,
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                if (_projectionReady) ...[
                  if (_projection != null) ...[
                    ...
                    if (_projection!.status != ViabilityStatus.feasible) ...[
                    ...
                    _RitualCard(
                      ...
                      onTimeChanged: (t)   => setState(() => _notificationTime = t),
                    ),
                    const SizedBox(height: 28),
                  ],
                ] else ...[
                ...
                _NoteTile(onTap: () async {
                  await Navigator.push(context, MaterialPageRoute(
                      builder: (_) => GoalNotesEditorScreen(goal: widget.goal)));
                }),
              ]),
            )],
          )),
      ...
      child: Column(children: [
        Row(children: [
          Container(
            ...
            child: const Center(child: Icon(
                Iconsax.notification_bing, size: 14, color: _kPurple)),
          ),
          const SizedBox(width: 10),
          Text('Activar recordatorio',
              style: _T.label(13, w: FontWeight.w700,
...
class _TimeTile extends StatefulWidget {
  ...
  @override State<_TimeTile> createState() => _TimeTileState();
}
        ...
        builder: (_, __) => Transform.scale(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
                ...
                border: Border.all(
                    color: _kPurple.withOpacity(0.25), width: 0.5)),
            child: Row(children: [
              Container(
                width: 30, height: 30,
                decoration: BoxDecoration(
                ...
                child: const Center(child: Icon(
                    Iconsax.clock, size: 14, color: _kPurple)),
              ),
              ...
              Expanded(child: Column(
                ...
                Text(timeStr, style: _T.label(14,
...
class _GoalContextCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isDark   = Theme.of(context).brightness == Brightness.dark;
    final onSurf   = Theme.of(context).colorScheme.onSurface;
    final bg       = isDark ? Colors.white.withOpacity(0.06) : Colors.black.withOpacity(0.03);
...
class _NameFieldState extends State<_NameField> {
  final _focus = FocusNode(); bool _hasFocus = false;
  @override void initState() { super.initState(); _focus.addListener(() => setState(() => _hasFocus = _focus.hasFocus)); }
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(14),
          border: _hasFocus ? Border.all(color: _kBlue.withOpacity(0.60), width: 1.5)
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: Padding(padding: const EdgeInsets.only(left: 14, right: 10),
              child: Icon(Iconsax.flag, size: 17,
    ...
    return AnimatedContainer(
      ...
      child: Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          Text('\$', style: _T.mono(22, c: hasVal ? _kBlue : onSurf.withOpacity(0.25), w: FontWeight.w400)),
          ...
          Expanded(child: TextFormField(
            ...
            decoration: InputDecoration(
              hintText: '0', hintStyle: _T.mono(28, c: onSurf.withOpacity(0.20)),
          ...
          Text('COP', style: _T.label(11, c: onSurf.withOpacity(0.28), w: FontWeight.w500)),
        ]),
      ),
    );
  }
...
class _DateTile extends StatefulWidget {
  final DateTime? date; final VoidCallback onTap, onClear;
...
class _DateTileState extends State<_DateTile> with SingleTickerProviderStateMixin {
  late final AnimationController _c = AnimationController(vsync: this, duration: const Duration(milliseconds: 70));
  @override void dispose() { _c.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) {
    final isDark  = Theme.of(context).brightness == Brightness.dark;
    ...
    final dateStr = hasDate ? DateFormat('d \'de\' MMMM, yyyy', 'es').format(widget.date!) : 'Seleccionar fecha';
    return GestureDetector(
      onTapDown:   (_) { _c.forward(); HapticFeedback.selectionClick(); },
      onTapUp:     (_) { _c.reverse(); widget.onTap(); },
      onTapCancel: ()  => _c.reverse(),
          ...
          child: Container(
            ...
            child: Row(children: [
              Container(width: 34, height: 34,
                  decoration: BoxDecoration(
                    color: hasDate ? _kBlue.withOpacity(0.10) : onSurf.withOpacity(0.06),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Center(child: Icon(Iconsax.calendar_1, size: 15,
              ...
              if (hasDate)
                GestureDetector(
                  ...
                  child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
                      child: Icon(Icons.close_rounded, size: 15, color: onSurf.withOpacity(0.28))),
                )
              else
                Icon(Icons.chevron_right_rounded, size: 17, color: onSurf.withOpacity(0.22)),
            ]),
          ),
        ),
      ),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text(p.status.title, style: _T.label(13, w: FontWeight.w700, c: statusColor)),
          const SizedBox(width: 6),
          Text('· ${p.status.subtitle}', style: _T.label(12, c: onSurf.withOpacity(0.45), w: FontWeight.w400)),
        ]),
        const SizedBox(height: 14),
        Row(children: [
          _ProjMetric(label: 'Mensual',    value: fmt.format(p.monthlyNeeded),                 color: statusColor),
          _ProjDot(c: onSurf),
          _ProjMetric(label: 'Meses',      value: '${p.monthsRemaining}',                      color: onSurf),
          _ProjDot(c: onSurf),
          _ProjMetric(label: 'Del ingreso', value: '${p.percentageOfIncome.toStringAsFixed(0)}%',
...
class _ProjMetric extends StatelessWidget {
  ...
  const _ProjMetric({required this.label, required this.value, required this.color});
  @override
  Widget build(BuildContext context) {
    final onSurf = Theme.of(context).colorScheme.onSurface;
    return Expanded(child: Column(children: [
      ...
      const SizedBox(height: 2),
      Text(label, style: _T.label(10, c: onSurf.withOpacity(0.40)), textAlign: TextAlign.center),
    ]));
  }
}
...
class _SuggestionsRow extends StatelessWidget {
  final FinancialProjection projection; final ValueChanged<RecommendationType> onApply;
  ...
  Widget build(BuildContext context) {
    ...
    final p      = projection;
    final items  = [
      _SuggestionItem(type: RecommendationType.extendDeadline,
          label: 'Extender plazo', detail: '+${p.suggestedMonths} meses',
          icon: Iconsax.calendar_add, color: _kBlue),
      _SuggestionItem(type: RecommendationType.increaseContribution,
          label: 'Aumentar aporte', detail: fmt.format(p.monthlyNeeded * 1.5),
          icon: Iconsax.arrow_up_1, color: _kGreen, readOnly: true),
      _SuggestionItem(type: RecommendationType.reduceTarget,
          label: 'Reducir objetivo', detail: '-20% (${fmt.format(p.remainingAmount * 0.8)})',
          icon: Iconsax.arrow_down_1, color: _kOrange),
    ];
    return Container(
      decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(14)),
      child: Column(mainAxisSize: MainAxisSize.min,
        children: items.indexed.map((e) {
          final (i, item) = e;
          return _SuggestionTile(item: item, showDivider: i < items.length - 1,
...
class _SuggestionItem {
  final RecommendationType type; final String label, detail;
  final IconData icon; final Color color; final bool readOnly;
  const _SuggestionItem({required this.type, required this.label,
      required this.detail, required this.icon, required this.color,
      this.readOnly = false});
}

class _SuggestionTile extends StatefulWidget {
  final _SuggestionItem item; final bool showDivider; final VoidCallback? onApply;
  const _SuggestionTile({required this.item, required this.showDivider, this.onApply});
...
class _SuggestionTileState extends State<_SuggestionTile> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final onSurf = Theme.of(context).colorScheme.onSurface;
    final item   = widget.item; final canTap = widget.onApply != null;
    return GestureDetector(
      onTapDown:   canTap ? (_) { _c.forward(); HapticFeedback.selectionClick(); } : null,
        ...
        builder: (_, __) => Opacity(
          ...
          child: Column(children: [
            Padding(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 13),
                child: Row(children: [
                  Container(width: 34, height: 34,
                      decoration: BoxDecoration(color: item.color.withOpacity(0.10),
                          borderRadius: BorderRadius.circular(10)),
                      child: Center(child: Icon(item.icon, size: 15, color: item.color))),
                  const SizedBox(width: 12),
                  Expanded(child: Text(item.label,
                      style: _T.label(14, w: FontWeight.w600, c: onSurf))),
                  Text(item.detail, style: _T.mono(12,
                      c: canTap ? item.color : onSurf.withOpacity(0.40),
                      w: FontWeight.w600)),
                  if (canTap) ...[
                    const SizedBox(width: 6),
                    Icon(Icons.chevron_right_rounded, size: 15, color: onSurf.withOpacity(0.22)),
                  ],
                ])),
            if (widget.showDivider)
              Padding(padding: const EdgeInsets.only(left: 14 + 34 + 12),
                  child: Container(height: 0.5, color: onSurf.withOpacity(0.07))),
          ]),
        ),
      ),
    );
...
class _PriorityTiles extends StatelessWidget {
  final GoalPriority selected; final ValueChanged<GoalPriority> onChanged;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: GoalPriority.values.map((p) {
        final isSel = p == selected; final color = _kPriorityColors[p]!;
        return Expanded(child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: isSel && !isDark ? [BoxShadow(
                  color: Colors.black.withOpacity(0.08),
                  blurRadius: 6, offset: const Offset(0, 2))] : null,
            ),
            child: Text(_kPriorityLabels[p]!, textAlign: TextAlign.center,
                style: _T.label(13,
...
class _TimeframeTiles extends StatelessWidget {
  final GoalTimeframe selected; final ValueChanged<GoalTimeframe> onChanged;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(3),
      decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(12)),
      child: Row(children: GoalTimeframe.values.map((t) {
        ...
        return Expanded(child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Text(_kTimeframeLabels[t]!, textAlign: TextAlign.center,
                style: _T.label(13,
                    w: isSel ? FontWeight.w700 : FontWeight.w500,
...
class _CategoryTrigger extends StatefulWidget {
  final List<Category>? categories; final String? selectedId; final VoidCallback onTap;
  const _CategoryTrigger({required this.categories, required this.selectedId, required this.onTap});
  @override State<_CategoryTrigger> createState() => _CategoryTriggerState();
}
class _CategoryTriggerState extends State<_CategoryTrigger> with SingleTickerProviderStateMixin {
  late final AnimationController _c = AnimationController(vsync: this, duration: const Duration(milliseconds: 70));
  @override void dispose() { _c.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTapDown:   loading ? null : (_) { _c.forward(); HapticFeedback.selectionClick(); },
        ...
        builder: (_, __) => Transform.scale(
          ...
          child: Container(
            ...
            child: loading
                ? Row(children: [
                    Container(width: 34, height: 34,
                        decoration: BoxDecoration(color: onSurf.withOpacity(0.06),
                            borderRadius: BorderRadius.circular(10))),
                    const SizedBox(width: 12),
                ...
                : Row(children: [
                    Container(width: 34, height: 34,
                        decoration: BoxDecoration(color: color.withOpacity(0.10),
                            borderRadius: BorderRadius.circular(10)),
                        child: Center(child: Icon(cat?.icon ?? Iconsax.category,
                            size: 15, color: color))),
                    const SizedBox(width: 12),
                    Expanded(child: Text(cat?.name ?? 'Seleccionar categoría',
                        style: _T.label(14, w: FontWeight.w600,
                            c: cat != null ? onSurf : onSurf.withOpacity(0.38)))),
                    Icon(Icons.chevron_right_rounded, size: 17, color: onSurf.withOpacity(0.22)),
                  ]),
          ),
        ),
...
class _CategorySheet extends StatelessWidget {
  final List<Category> categories; final String? selectedId;
  final ScrollController scrollController; final ValueChanged<Category> onSelected;
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRRect(
      ...
      child: BackdropFilter(
        filter: ImageFilter.blur(sigmaX: 20, sigmaY: 20),
        child: Container(color: bg, child: Column(children: [
          const SizedBox(height: 8),
          Container(width: 36, height: 4,
              decoration: BoxDecoration(color: onSurf.withOpacity(0.15),
                  borderRadius: BorderRadius.circular(2))),
          const SizedBox(height: 16),
...
class _CatRow extends StatefulWidget {
  final Category category; final bool isSelected; final VoidCallback onTap;
  const _CatRow({required this.category, required this.isSelected, required this.onTap});
          ...
          child: Padding(padding: const EdgeInsets.symmetric(vertical: 12),
              child: Row(children: [
                Container(width: 34, height: 34,
                    decoration: BoxDecoration(color: color.withOpacity(0.10),
                        borderRadius: BorderRadius.circular(10)),
                    child: Center(child: Icon(widget.category.icon ?? Iconsax.category,
                        size: 15, color: color))),
                const SizedBox(width: 12),
                ...
                AnimatedOpacity(
                  ...
                  duration: const Duration(milliseconds: 200),
                  child: const Icon(Icons.check_rounded, size: 17, color: _kBlue),
                ),
              ])),
        ),
      ),
...
class _NoteTileState extends State<_NoteTile> with SingleTickerProviderStateMixin {
  late final AnimationController _c = AnimationController(vsync: this, duration: const Duration(milliseconds: 70));
  @override void dispose() { _c.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) {
      ...
      child: AnimatedBuilder(
        ...
        builder: (_, __) => Opacity(
          ...
          child: Container(
            ...
            child: Row(children: [
              Container(width: 34, height: 34,
                  decoration: BoxDecoration(color: _kBlue.withOpacity(0.10),
                      borderRadius: BorderRadius.circular(10)),
                  child: const Center(child: Icon(
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('Añade enlaces, ideas y apuntes importantes',
                    style: _T.label(11,
                        c: Theme.of(context).colorScheme.onSurface.withOpacity(0.40),
                        w: FontWeight.w400)),
              ])),
...
class _ConfirmSheet extends StatelessWidget {
  final Goal goal; final String newName; final double newAmount;
  final DateTime? newDate; final bool nameChanged, amountChanged, dateChanged;
  final FinancialProjection? projection;
  const _ConfirmSheet({required this.goal, required this.newName,
      required this.newAmount, required this.newDate,
      required this.nameChanged, required this.amountChanged,
      required this.dateChanged, this.projection});
  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final onSurf = Theme.of(context).colorScheme.onSurface;
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Revisa antes de guardar', style: _T.label(13,
                c: onSurf.withOpacity(0.45), w: FontWeight.w400)),
            const SizedBox(height: 20),
            if (nameChanged) _ChangeRow(label: 'Nombre', oldValue: goal.name,
                newValue: newName, onSurf: onSurf),
            if (amountChanged) _ChangeRow(label: 'Monto',
                oldValue: fmt.format(goal.targetAmount),
                newValue: fmt.format(newAmount), onSurf: onSurf),
            if (dateChanged) _ChangeRow(label: 'Fecha',
                oldValue: goal.targetDate != null
                    ? dateF.format(goal.targetDate!) : 'Sin fecha',
                newValue: newDate != null ? dateF.format(newDate!) : 'Sin fecha',
                onSurf: onSurf),
            if (projection != null) ...[
              const SizedBox(height: 12),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                decoration: BoxDecoration(color: _kBlue.withOpacity(0.07),
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(
                    ...
                    style: _T.label(12, c: onSurf.withOpacity(0.60), w: FontWeight.w400),
                  )),
                ]),
              ),
            ],
            const SizedBox(height: 24),
            _SheetBtn(label: 'Guardar cambios', color: _kBlue,
                onTap: () => Navigator.pop(context, true)),
            const SizedBox(height: 10),
            _SheetBtn(label: 'Cancelar', color: onSurf.withOpacity(0.08),
                textColor: onSurf, onTap: () => Navigator.pop(context, false)),
          ]),
        ),
      ),
    );
...
class _ChangeRow extends StatelessWidget {
  final String label, oldValue, newValue; final Color onSurf;
  const _ChangeRow({required this.label, required this.oldValue,
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(oldValue,
          style: _T.label(13, c: onSurf.withOpacity(0.38), w: FontWeight.w400)
              .copyWith(decoration: TextDecoration.lineThrough),
          overflow: TextOverflow.ellipsis)),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 6),
          child: const Icon(Icons.arrow_forward_rounded, size: 13, color: _kBlue)),
...
class _GroupLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final onSurf = Theme.of(context).colorScheme.onSurface;
    return Text(text, style: _T.label(11, w: FontWeight.w700,
...
class _SheetBtn extends StatefulWidget {
  final String label; final Color color; final Color? textColor; final VoidCallback onTap;
  const _SheetBtn({required this.label, required this.color,
      required this.onTap, this.textColor});
  @override State<_SheetBtn> createState() => _SheetBtnState();
}
class _SheetBtnState extends State<_SheetBtn> with SingleTickerProviderStateMixin {
  late final AnimationController _c = AnimationController(vsync: this, duration: const Duration(milliseconds: 80));
  @override void dispose() { _c.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) {
    return GestureDetector(
        ...
        builder: (_, __) => Transform.scale(
          ...
          child: Container(
            ...
            child: Center(child: Text(widget.label,
                style: _T.label(16, c: widget.textColor ?? Colors.white,
                    w: FontWeight.w700))),
          ),
        ),
      ),
...
class _SaveBtnState extends State<_SaveBtn> with SingleTickerProviderStateMixin {
  late final AnimationController _c = AnimationController(vsync: this, duration: const Duration(milliseconds: 80));
  @override void dispose() { _c.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) {
    ...
    return ClipRect(child: BackdropFilter(
      ...
      child: Container(
        ...
        child: GestureDetector(
          onTapDown: (_) { if (!widget.loading) { _c.forward(); HapticFeedback.mediumImpact(); } },
...
class _FrequencySelector extends StatelessWidget {
  final GoalSavingsFrequency? selected; final Function(GoalSavingsFrequency?) onChanged;
  const _FrequencySelector({required this.selected, required this.onChanged});
  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final onSurf = Theme.of(context).colorScheme.onSurface;
    return Row(children: GoalSavingsFrequency.values.map((freq) {
      ...
      return Expanded(child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: isSelected ? _kPurple.withOpacity(0.12)
          ...
          child: Center(child: Text(label, style: _T.label(12,
...
class _DaySelector extends StatelessWidget {
  final GoalSavingsFrequency frequency; final int? selectedDay; final Function(int) onDayChanged;
    ...
    if (frequency == GoalSavingsFrequency.weekly) {
      ...
      return Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
          ...
          final day = i + 1; final isSel = selectedDay == day;
          return GestureDetector(
            onTap: () { HapticFeedback.selectionClick(); onDayChanged(day); },
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 150),
              width: 36, height: 36,
              decoration: BoxDecoration(
                color: isSel ? _kPurple.withOpacity(0.12) : Colors.transparent,
                shape: BoxShape.circle,
                border: isSel ? Border.all(color: _kPurple.withOpacity(0.4)) : null,
              ),
              child: Center(child: Text(days[i], style: _T.label(13,
          ...
          child: Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(
                ...
                style: _T.label(13, w: FontWeight.w500,
                    c: selectedDay != null ? onSurf : onSurf.withOpacity(0.5)),
              ),
...
class FinancialProjection {
  ...
  FinancialProjection({
    required this.remainingAmount, required this.monthsRemaining,
    required this.monthlyNeeded,   required this.percentageOfIncome,
    required this.status,          required this.suggestedMonths,
  });
...
static FinancialProjection calculate({
    ...
    int months = (days / 30.437).ceil();
    if (months <= 0) months = 1;
    
    final daily   = remainingAmount / days;
    final monthly = daily * 30.437;
    final pct     = (monthly / userMonthlyIncome) * 100;

    return FinancialProjection(
      remainingAmount:    remainingAmount,
      monthsRemaining:    months, // 👈 Y la usamos aquí
